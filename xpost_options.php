	...
	if( $_POST['newBlog'] || $_POST['updateBlog'] ) {
		...
		if( count( $errors ) == 0 ) {
			...
			foreach( $response as $blog ) {
				...
				if( $_POST['newBlog'] ) {
					$rowsAffected = $wpdb->insert( XPOST_TABLE_NAME,
						array(
							'blogid' => $insBlogId,
							'selected' => $selected,
							'name' => $name,
					...
					if( $blogId == $insBlogId ) {
						$rowsAffected = $wpdb->update( XPOST_TABLE_NAME,
							array(
								'selected' => $selected,
								'name' => $name,
								'url' => $url,
		...
		$id = intval( $_POST['id'] );
		$sql = "SELECT blogid, xmlrpc, user, password, xpost_comments, xpost_community_server, xpost_summary_only, comment, selected FROM ".XPOST_TABLE_NAME." WHERE id = $id";
		...
		$id = intval( $_POST['id'] );
		$sql = "DELETE FROM ".XPOST_TABLE_NAME." WHERE id = $id";
		if( $wpdb->query( $sql ) <= 0 ) {
 ...
 * @param bool   $xpost_comments Selected for crossposting comments by default.
 * @param bool   $xpost_community_server Is a Community Server blog.
 * @param bool   $xpost_summary_only Just crosspost summaries.
...
function echo_edit_blog_tbl( $xmlrpc, $user, $password, $selected, $xpost_comments ,$xpost_community_server, $xpost_summary_only, $comment ) { ?>
	<table class="form-table">
		...
		<tr valign="top">
			...
			<td><fieldset><legend class="screen-reader-text"><span><?php _e('Community Server blog', 'xpost'); ?></span></legend>
			...
			<td><fieldset><legend class="screen-reader-text"><span><?php _e('Summary only', 'xpost'); ?></span></legend>
